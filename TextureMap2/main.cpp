...
void drawTextureMaps(Mat &rgbImage,Mat &pointCloud_XYZ){
	...
	glTexImage2D(GL_TEXTURE_2D,0,GL_RGB,KINECT_DEPTH_WIDTH-1,KINECT_DEPTH_HEGIHT,0,GL_RGB,GL_UNSIGNED_BYTE,rgbImage.data);
	glEnable(GL_TEXTURE_2D);

	for(y = 0;y < KINECT_DEPTH_HEGIHT - 1;y++){
		for(x = 0;x < KINECT_DEPTH_WIDTH - 1;x++){ 
			//奥行きが取得できてなかったら何もしない
			if(point[0]->z == 0){
				point[1]++,point[0]++;
				continue;
			...
			float u1 = (float)(x) / KINECT_DEPTH_WIDTH;
			float u2 = (float)(x+1) / KINECT_DEPTH_WIDTH;
			float v1 = (float)y / KINECT_DEPTH_HEGIHT;
			float v2 = (float)(y+1) / KINECT_DEPTH_HEGIHT;
			glBegin(GL_TRIANGLE_STRIP);
			//左上
			glTexCoord2f(u1, v1);
			glVertex3fv((GLfloat*)point[0]);
			point[0] += 1;
			//左下
			glTexCoord2f(u2, v1);
			glVertex3fv((GLfloat*)point[1]);
			point[1] += 1;	
			//右上
			glTexCoord2f(u1, v2);
			glVertex3fv((GLfloat*)point[0]);
